...
calculateLDRs <- function(mergedcounts,mergedstarts,noreplicates,refsequence) {
    ...
    return(list("LDR_C" = varStab$LDR_C, "LDR_CT" = varStab$LDR_CT, "stretches" = stretches, "nuclSelection" = nuclSelection, "nuclPosition" =  nuclPosition, "stretches" =  stretches, "processed_se" = se,
                "rawdors" = mergeddors ))
}
...
DOR_Mrd1 <- outputlogdropoffserb1[["processed_se"]]@assays@data@listData[["dropoff_rate"]]
raw_DOR_Mrd1 <- outputlogdropoffserb1[["rawdors"]]
write.table(DOR_Mrd1, file="scaled_DORs_Mrd1.txt", sep = "\t", quote = FALSE, row.names=TRUE, col.names=TRUE)
